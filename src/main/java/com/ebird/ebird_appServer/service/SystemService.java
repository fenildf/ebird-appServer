...
//	@Resource
	 ...
	 * @return
	 */
	public UserCur getCurrentUser(HttpServletRequest request) {
		HttpSession  session = request.getSession();
		if(session == null) {
			return null;
		}
		String token = request.getHeader("token");
		UserCur  user = (UserCur)session.getAttribute(token);
		return user;
	}
	 ...
	 * @throws Exception 
	 */
	public UserCur getCurrentUserForInterrupt() throws Exception {
		UserCur user = getCurrentUser();
		 if(user == null) {
